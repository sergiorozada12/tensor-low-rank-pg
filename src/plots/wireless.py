...
def plot_wireless():
    ...
    with plt.style.context(['science'], ['ieee']):
        ...
        plt.plot(med, label='RBF-PPO - 1,202 params.', color='y')
        plt.fill_between(range(p25.shape[0]), p25, p75, alpha=.2, color='y')

        med = np.median(wireless_nn, axis=0)
        p25 = np.percentile(wireless_nn, q=25, axis=0)
        p75 = np.percentile(wireless_nn, q=75, axis=0)
        plt.plot(med, label='NN-PPO - 1,093 params.', color='orange')
        plt.fill_between(range(p25.shape[0]), p25, p75, alpha=.2, color='orange')
        ...
        plt.plot(med, label='NN-PPO - 2,181 params.', color='g')
        ...
        plt.fill_between(range(p25.shape[0]), p25, p75, alpha=.2, color='r')
