...
class GoddardEnvContinuous(gym.Env):

    metadata = {"render.modes": ["human", "rgb_array"], "video.frames_per_second": 30}

    def __init__(self, rocket=Default()):
        super(GoddardEnvContinuous, self).__init__()
        ...
        self.action_space = gym.spaces.Box(
            low=np.array([0.0]), high=np.array([1.0]), shape=(1,), dtype=float
        )
    ...
    def step(self, action):
        ...
        thrust = np.clip(a, 0.0, self._r.THRUST_MAX)
        ...
        if is_done:
            reward = 1_000 * (self._h_max - self._r.H0)
        else:
            reward = 0.0
        ...
        return self._observation(), reward, is_done, extras, None
...
class GoddardEnvDiscrete(GoddardEnvContinuous):
    def step(self, action):
        v, h, m = self._state
        is_tank_empty = m <= self._r.M1

        a = 0.0 if is_tank_empty else float(action) / 9.0
        thrust = self._r.THRUST_MAX * a
        ...
        extras = dict(zip(self.extras_labels(), [action, thrust, drag, g]))
